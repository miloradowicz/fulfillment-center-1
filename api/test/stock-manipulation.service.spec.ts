    ...
    products: [
      {
        product: productId,
        amount: 10,
        equals: function(id) { return this.product.equals(id); }
      }
    ],
    defects: [
      {
...
describe('StockManipulationService', () => {
  ...
  describe('increaseProductStock', () => {
    it('should increase amount of existing product', async () => {
      ...
      await service.increaseProductStock(stockId, [
        { product: productId, amount: 5 } as ProductWithAmount,
      ]);
    ...
    it('should add new product if not exists', async () => {
      ...
      // Вызываем метод сервиса
      await service.increaseProductStock(stockId, [
        { product: productId2, amount: 7 } as ProductWithAmount,
      ]);
    ...
    it('should throw NotFoundException if stock not found', async () => {
      ...
      await expect(
        service.increaseProductStock(stockId, [
          { product: productId, amount: 5 } as ProductWithAmount,
        ]),
    ...
    it('should use cached stock for multiple calls', async () => {
      ...
      // Делаем второй вызов
      await service.increaseProductStock(stockId, [
        { product: productId, amount: 3 } as ProductWithAmount,
      ]);
  ...
  describe('decreaseProductStock', () => {
    it('should decrease amount of existing product', async () => {
      ...
      // Вызываем метод сервиса
      await service.decreaseProductStock(stockId, [
        { product: productId, amount: 3 } as ProductWithAmount,
      ]);
    ...
    it('should add new product with negative amount if not exists', async () => {
      ...
      // Вызываем метод сервиса
      await service.decreaseProductStock(stockId, [
        { product: productId2, amount: 4 } as ProductWithAmount,
      ]);
    ...
    it('should throw NotFoundException if stock not found', async () => {
      ...
      // Проверяем, что метод выбрасывает исключение
      await expect(
        service.decreaseProductStock(stockId, [
          { product: productId, amount: 3 } as ProductWithAmount,
        ]),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('increaseDefectStock', () => {
    it('should increase amount of existing defect product', async () => {
      ...
      // Вызываем метод сервиса
      await service.increaseDefectStock(stockId, [
        { product: productId, amount: 3 } as ProductWithAmount,
      ]);
    ...
    it('should add new defect product if not exists', async () => {
      ...
      // Вызываем метод сервиса
      await service.increaseDefectStock(stockId, [
        { product: productId2, amount: 2 } as ProductWithAmount,
      ]);
    ...
    it('should throw NotFoundException if stock not found', async () => {
      ...
      // Проверяем, что метод выбрасывает исключение
      await expect(
        service.increaseDefectStock(stockId, [
          { product: productId, amount: 2 } as ProductWithAmount,
        ]),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('decreaseDefectStock', () => {
    it('should decrease amount of existing defect product', async () => {
      ...
      // Вызываем метод сервиса
      await service.decreaseDefectStock(stockId, [
        { product: productId, amount: 2 } as ProductWithAmount,
      ]);
    ...
    it('should add new defect product with negative amount if not exists', async () => {
      ...
      // Вызываем метод сервиса
      await service.decreaseDefectStock(stockId, [
        { product: productId2, amount: 3 } as ProductWithAmount,
      ]);
    ...
    it('should throw NotFoundException if stock not found', async () => {
      ...
      // Проверяем, что метод выбрасывает исключение
      await expect(
        service.decreaseDefectStock(stockId, [
          { product: productId, amount: 2 } as ProductWithAmount,
        ]),
      ).rejects.toThrow(NotFoundException);
    });
