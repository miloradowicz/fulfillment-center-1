import { Stock, StockWriteOffWithPopulatedProducts } from '@/types'
...
interface Props {
  selector: (stock: Stock) => StockWriteOffWithPopulatedProducts[]
}
...
const StockProductsDataList: FC<Props> = ({ selector }) => {
  ...
  const columns: ColumnDef<StockWriteOffWithPopulatedProducts>[] = [
    {
      accessorKey: 'client',
      ...
      cell: ({ row }) => {
        return (
          <div className="flex items-center gap-2">
            ...
            {row.original.reason && <TooltipProvider>
              <Tooltip>
